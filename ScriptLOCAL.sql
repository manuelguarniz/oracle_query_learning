...
BEGIN
    FIND_EMPLOYEE_BY_ID(5, C_EMPLEADOS);
    LOOP
        FETCH C_EMPLEADOS INTO ROW_EMPLEADO;
        EXIT WHEN C_EMPLEADOS%NOTFOUND;
        
        --SELECT ROW_EMPLEADO.NOMBRE FROM DUAL;
        dbms_output.put_line(ROW_EMPLEADO.NOMBRE||ROW_EMPLEADO.SALARIO);
...
dbms_output.put_line('Agregar dï¿½a: '||(HOY + 1));
dbms_output.put_line('Dï¿½a: '||TO_CHAR(HOY, 'dd'));
dbms_output.put_line('Mes: '||TO_CHAR(HOY, 'mm'));
dbms_output.put_line('Aï¿½o: '||TO_CHAR(HOY, 'yyyy'));
